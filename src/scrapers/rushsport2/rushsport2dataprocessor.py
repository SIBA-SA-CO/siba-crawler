...
class RushSport2DataProcessor(IDataProcessor):
    ...
    def processData(self, rawData, defaultDescription: str, targetDate: str) -> list:
        ...
        for _, row in rawData.iterrows():
            try:
                ...
                processedEvents.append(self._build_event(eventDate, eventTime, eventTitle, eventContent))
            except Exception:
                continue
    ...
    def _parse_date(self, value):
        ...
        if isinstance(value, str):
            parsedFromString = self._parse_date_string(value.strip())
            if parsedFromString is not None:
    ...
    def _parse_time(self, value):
        ...
        if isinstance(value, (int, float)) and 0 <= value < 1:
            totalSeconds = int(round(value * 24 * 60 * 60))
            return self._time_from_seconds(totalSeconds)
    ...
    def _time_from_seconds(self, totalSeconds: int) -> time:
